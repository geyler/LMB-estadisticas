...
// Allow demo seed if requested via key, or if logged in as super_admin, or if 0 users exist
$key = $_GET['key'] ?? '';
$stmtCount = $pdo->query("SELECT COUNT(*) as total FROM users");
$totalUsers = $stmtCount->fetch()['total'];

if ($totalUsers > 0 && $key !== 'lmb2026') {
    if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'super_admin') {
        // Return clear JSON with instruction
        echo json_encode([
            'success' => false, 
            'message' => 'Acceso restringido. Inicia sesión como Super Admin o usa el parámetro ?key=lmb2026 para activar la demo.'
        ]);
        exit;
    }
}
...
try {
    ...
    // Clear auto-created default rows
    ...
    $stadiumsData = [
        ['Estadio Nacional Ezeiza', 'Campo Principal #1', 'Ezeiza, Prov. Buenos Aires'],
        ['Polideportivo Belgrano', 'Diamante Central A', 'Belgrano, CABA'],
        ['Parque Chacabuco Béisbol', 'Cancha Principal', 'Chacabuco, CABA'],
        ['Club San Martín Béisbol', 'Estadio de San Martín', 'San Martín, Prov. Buenos Aires'],
        ['Complejo Villa Domínico', 'Cancha Sur #2', 'Avellaneda, Prov. Buenos Aires'],
        ['Estadio La Plata Béisbol', 'Diamante Olímpico', 'La Plata, Prov. Buenos Aires'],
        ['Campo Deportivo Lanús', 'Diamante #1', 'Lanús, Prov. Buenos Aires']
    ];
    ...
    // Helper to simulate finished games
    function simulateGame($pdo, $seasonId, $catId, $homeTeamId, $awayTeamId, $stadiumId, $dateStr, $stageStr, $playersByTeam) {
        $homeScore = rand(2, 9);
        $awayScore = rand(1, 8);
        ...
        $stmtG->execute([$seasonId, $catId, $homeTeamId, $awayTeamId, $stadiumId, $dateStr, $stageStr, $homeScore, $awayScore, $homeHits, $awayHits, $homeErrors, $awayErrors, "Encuentro oficial disputado con alta intensidad."]);
        ...
        for ($inn = 1; $inn <= 9; $inn++) {
            $hr = ($inn === 9) ? $hLeft : rand(0, min(2, $hLeft));
            $hLeft -= $hr;
            $stmtLS->execute([$gameId, $homeTeamId, $inn, $hr]);

            $ar = ($inn === 9) ? $aLeft : rand(0, min(2, $aLeft));
            $aLeft -= $ar;
    ...
    for ($r = 1; $r <= 3; $r++) {
        for ($i = 0; $i < count($a1Teams); $i++) {
            for ($j = $i + 1; $j < count($a1Teams); $j++) {
                $gDate->modify('+7 days');
                simulateGame($pdo, $season2025Id, $cat2025A1, $a1Teams[$i], $a1Teams[$j], $stadiumIds[$i], $gDate->format('Y-m-d H:i:s'), 'Temporada Regular', $playersByTeam);
            }
        }
    }

    $gDate->modify('+7 days');
    simulateGame($pdo, $season2025Id, $cat2025A1, $teamIds[0], $teamIds[1], $stadiumIds[0], $gDate->format('Y-m-d H:i:s'), 'Gran Final A1', $playersByTeam);
    ...
    for ($r = 1; $r <= 2; $r++) {
        for ($i = 0; $i < count($a2Teams); $i++) {
            for ($j = $i + 1; $j < count($a2Teams); $j++) {
                $gDate->modify('+7 days');
                simulateGame($pdo, $season2025Id, $cat2025A2, $a2Teams[$i], $a2Teams[$j], $stadiumIds[$i+4], $gDate->format('Y-m-d H:i:s'), 'Temporada Regular', $playersByTeam);
            }
        }
    }
    $gDate->modify('+7 days');
    simulateGame($pdo, $season2025Id, $cat2025A2, $teamIds[4], $teamIds[5], $stadiumIds[4], $gDate->format('Y-m-d H:i:s'), 'Gran Final A2', $playersByTeam);
    $stmtChamp->execute([$season2025Id, $cat2025A2, $teamIds[4], 'Campeón Oficial A2', 'Ganador de la Serie Final 2025']);

    // 9. Generate 2026 Games
    $gDate2026 = new DateTime('2026-03-01 14:00:00');

    for ($i = 0; $i < count($a1Teams); $i++) {
        for ($j = $i + 1; $j < count($a1Teams); $j++) {
            $gDate2026->modify('+7 days');
            simulateGame($pdo, $season2026Id, $cat2026A1, $a1Teams[$i], $a1Teams[$j], $stadiumIds[$i], $gDate2026->format('Y-m-d H:i:s'), 'Temporada Regular', $playersByTeam);
        }
    }
    ...
    for ($i = 0; $i < count($a2Teams); $i++) {
        for ($j = $i + 1; $j < count($a2Teams); $j++) {
            $gDate2026->modify('+7 days');
            simulateGame($pdo, $season2026Id, $cat2026A2, $a2Teams[$i], $a2Teams[$j], $stadiumIds[$i+4], $gDate2026->format('Y-m-d H:i:s'), 'Temporada Regular', $playersByTeam);
        }
    }
    ...
    echo json_encode([
        'success' => true,
        'message' => '¡Demostración de la LMB activada con éxito! Se cargaron las Temporadas 2025 y 2026, 7 equipos, 73 jugadores, sedes y estadísticas completas.'
    ]);
} catch (Exception $e) {
    echo json_encode([
        'success' => false,
        'message' => 'Error al generar la demostración: ' . $e->getMessage()
    ]);
}
